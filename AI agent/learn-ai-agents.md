...
The first project is already in this workspace:

1. Open [`START-HERE.md`](START-HERE.md).
2. Follow [`01-hello-llm/README.md`](01-hello-llm/README.md): venv, `.env` with one API key.
3. Run `python main.py --style tutor` and `python main.py --style poet`.
4. Run `python chat.py` so you see conversation history.
5. Rewrite [`01-hello-llm/notes.md`](01-hello-llm/notes.md) in your own words: message, token, tool.

That is Stage A. Stage B (`02-json-clerk`) comes next.
